...
class DataGenerator(keras.utils.Sequence):
  ...
  def __data_generation_with_additional_infomation(self, data_IDs_temp):
    ...
    B = np.zeros((a*self.batch_size, 64, 64, 64, 23),dtype=np.single)
    C = np.zeros((a*self.batch_size, 32, 32, 32, 23),dtype=np.single)
    D = np.zeros((a*self.batch_size, 16, 16, 16, 23),dtype=np.single)
    E = np.zeros((a*self.batch_size, 8, 8, 8, 23),dtype=np.single)
    ...
    for k in range(self.batch_size):
      ...
      for j,name in enumerate(names1):
        gx = np.fromfile(self.dpath+'16_subsampled/'+name+'_16subsampled/'+str(data_IDs_temp[k]),dtype=np.single)
        gx = np.reshape(gx,(8,8,8))
        gx = dataprocess(gx)
        E[c+0,:,:,:,j] = gx
