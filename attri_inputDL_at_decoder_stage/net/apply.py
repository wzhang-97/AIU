...
def loadModel():
  global model
  model = load_model(path+"../../../test11/check11/fseg-85.hdf5")
def godfb(): 
  name0 = 'nx'
  ...
  fx = np.transpose(fx)
  fx.tofile(path+"test11_dfb_cut.dat",format="%4")
...
def gohz08(): 
  ...
  fx = goPredictSubs2(m1,m2,m3,gms,gma)#make a prediction 
  fx = np.transpose(fx)
  fx.tofile(path+"test11_hz08_cut.dat",format="%4")
...
def govolve(): 
  ...
  fx = goPredictSubs2(m1,m2,m3,gms,gma)#make a prediction 
  fx = np.transpose(fx)
  fx.tofile(path+"test11_volve.dat",format="%4")
...
def gonew(): 
  ...
  fx = goPredictSubs2(m1,m2,m3,gms,gma)#make a prediction 
  fx = np.transpose(fx)
  fx.tofile(path+"test11_new.dat",format="%4")
...
def gobgp_subsampled(): 
  ...
  fx = np.transpose(fx)
  fx.tofile(path+"test11_bgp_cut_z_2_subsampled_6223.dat",format="%4")
  ...
  fx = np.transpose(fx)
  fx.tofile(path+"test11_bgp_cut_z_4_subsampled_6223.dat",format="%4")
...
def gof3(): 
  ...
  fx = goPredictSubs2(m1,m2,m3,gms,gma)#make a prediction 
  fx = np.transpose(fx)
  fx.tofile(path+"test11_f3d_cut.dat",format="%4")
...
def gobgp(): 
  ...
  fx = goPredictSubs2(m1,m2,m3,gms,gma)#make a prediction 
  fx = np.transpose(fx)
  fx.tofile(path+"test11_bgp_cut6223.dat",format="%4")
  ...
  fx = goPredictSubs2(m1,m2,m3,gms,gma)#make a prediction 
  fx = np.transpose(fx)
  fx.tofile(path+"test11_bgp_cut18226.dat",format="%4")
  ...
  fx = goPredictSubs2(m1,m2,m3,gms,gma)#make a prediction 
  fx = np.transpose(fx)
  fx.tofile(path+"test11_bgp_cut24228.dat",format="%4")
...
def gokerry(): 
  ...
  fx = goPredictSubs2(m1,m2,m3,gms,gma)#make a prediction 
  fx = np.transpose(fx)
  fx.tofile(path+"test11_kerry_cut.dat",format="%4")
...
def gofakedata(): 
  ...
  for i in range(start_i, end_i):
    ...
    fx = np.transpose(fx)
    output_file = path + "test11_fake" + str(i) + ".dat"
  ...
  for k3 in range(c3):
    for k2 in range(c2):
      for k1 in range(c1):
        ...
        fk = model.predict([gks,gka,gka2,gka4,gka8],verbose=1) #fault prediction
